# coffey.codes — BD profile (Bugsy's source of truth)

This is Bugsy's reference for everything BD/sales/outreach. Edit this
file when the business changes — every workflow that does prospect
research, outreach drafting, or lead scoring loads this in.

Source synthesis: `D:\repos\coffey.codes\docs\documentation\guides\marketing\*`
plus the deep-dive SEO doc and Anthony's stated priorities. The 4-ICP
breakdown from `uvp.md` is canonical (newer than the 2-ICP version in
...
with a number unless the prospect signals budget first.

## 3. ICPs (canonical — from `uvp.md`)

Listed in order of strategic priority for outbound focus.

### ICP 1 — Established SMEs (20–200 employees)
- **Industries:** SaaS, professional services, e-commerce, non-profits
- **Buyer titles:** CTO, VP Engineering, Head of Product, founder
- **Need:** custom web/mobile builds, legacy modernization, reliable long-term partner
...
- **Buyer titles:** owner, marketing lead, ops lead
...
- **Agencies wanting white-label cheap dev work** (vs. partnership/overflow) — race to the bottom

## 5. Positioning / UVP

**Master theme:** Dependable Transformation.
...
What prospects are feeling when they're ready for Anthony:
- Burned by unreliable freelancers; projects stalled; budget burned
- Legacy systems blocking growth or scaring engineers off
...
When Bugsy is doing prospect research, these are the green flags. Surface
them prominently in the brief and use them as conversation hooks in
drafted outreach.
...
5. **Hiring an "AI engineer" / "ML engineer" but the company is tiny (<30 ppl)** — they don't have the data infra to support a hire yet; consulting fits better

### Medium signals (worth a closer look)
6. **Open GitHub issues stale 30+ days on repos in Anthony's stack** (Node, Python, React, TS) — capacity shortage signal
...
## 8. Outreach voice (going OUT to prospects)

**Critical:** When Bugsy drafts cold outreach, social posts, or any
prospect-facing copy, the **mobster persona DROPS to zero**. The voice
becomes:

- **Professional, direct, anti-hype.** Match the prospect's formality.
- Personalized — reference a specific signal Bugsy found (a job post, a press release, something they said publicly)
- Concise — 4-6 sentences max for cold email
- Soft CTA — "20-min chat to see if there's a fit," never "book a sales call"
- Self-deprecating > self-aggrandizing — "I might be off-base here, but..."
- Never use words like "leverage," "synergize," "thought leader," "value-add," "circle back," "touch base"
- Lead with their problem, not Anthony's resume
- Sign as "Anthony" not "Anthony Coffey from coffey.codes" — it's a person reaching out, not a brand

Bugsy's mobster voice stays where it belongs: internal Slack messages
TO Anthony. The boss-facing wrapper. Outbound copy is just clean
professional writing.

## 9. CTAs Bugsy can offer

Default ladder (least → most committed):
1. **Free 20-min strategy call** — primary CTA for cold outreach
...
- `/lp/practical-ai`
- `/lp/smb-web-marketing`
- (plus 2 more under `app/lp/` — confirm titles when wiring up the link library)

## 10. Channels (where strategy is focused)

**Primary:** LinkedIn (organic + paid), SEO/blog content, ONE focused community (currently TBD — go deep, not wide).

**Secondary:** Reddit ads in `r/node`, `r/Python`, `r/webdev`, `r/Austin`, `r/smallbusiness`. Strategic agency partnerships. Referrals.

**Avoided:** Instagram, Facebook (B2C mismatch). YouTube (capacity gap).

When Bugsy suggests outreach channels for a specific prospect, prefer the channel where the prospect is already visible (e.g. CTO posts on LinkedIn → reach there; founder is on HN → engage there).
...
## How Bugsy uses this file

Every BD-related Bugsy workflow loads this file as part of its system
prompt context. When the user asks Bugsy to research a prospect, draft
outreach, or score a lead, Bugsy:

1. Reads this file's ICP, anti-ICP, and signals to qualify
2. Pulls voice/tone rules from §8 for any drafted copy
3. Falls back to flagged gaps in §11 honestly — never invents pricing,
   case studies, or capabilities not listed here
4. Defaults to the lowest-friction CTA from §9 unless the prospect's
   stated context warrants jumping higher
