...
| Date         | Decision                          | Why                                                                                          |
| ------------ | --------------------------------- | -------------------------------------------------------------------------------------------- |
| May 8, 2026 | Job-board scoring batches all new jobs into a single LiteLLM call (1→1 along the score path) instead of one HTTP request per job | Per-item iteration silently dropped 181 of 182 jobs in a real run — n8n's HTTP node + downstream Code nodes lost items somewhere in the chain. Single batched call sidesteps it, costs less (system prompt ships once), and is bounded by Haiku's 8k output budget. Trade-off: 100-job cap per run; backlog defers to next day. |
| May 7, 2026 | LLM-score every new job before storing; only insert ≥75 fit. Threshold lives in `Aggregate Scored` Code node, scoring criteria in `Build Score Request` system prompt | Job board was burying real fits under Java/Spring, .NET, financial-analyst noise. Filtering at fetch (haiku-4-5) is cheap and keeps the table signal-dense. Existing rows aren't backfilled — re-scoring is a manual one-shot if the rubric shifts. |
