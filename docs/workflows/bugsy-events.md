...
The intent is for [`bugsy.json` (unified)](bugsy-unified.md) to eventually own all Slack surfaces — `bugsy-events` + `bugsy-chat` + `bugsy-slack-rag` rolled into one. Until the unified flow is activated in n8n, `bugsy-events` stays the live handler for `/slack-bugsy`. The two workflows can't both be active — Slack's webhook path would collide.
