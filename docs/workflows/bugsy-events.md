...
`bugsy-events` is **inactive** in n8n — the [unified workflow](bugsy-unified.md) took over `/slack-bugsy` along with `/ask`, with the same memory store and added RAG injection. This JSON stays in the repo as historical reference.
