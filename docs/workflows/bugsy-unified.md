...
**Active:** `false` · **Nodes:** 23 · **Execution order:** `v1`
...
- notion (read + page-create): for anything in the boss's Notion workspace — his Trello-style kanban project board, editorial/SEO content (calendars, content disposition, content strategy), personal notes and journal, reference/wiki docs. Use it whenever the question wants CURRENT Notion content: 'what's queued in next week's editorial calendar?', 'what's on my kanban this week?', 'what did I journal about Friday?', 'find my notes on X', etc. The KNOWLEDGE BASE may contain ingested Notion snapshots from the daily refresh cron — those are point-in-time and stale within hours, so prefer the live tool when the question is about current state. The Notion API surfaces pages, blocks, and 'data sources' (databases). When you call the tool for reads, cite specifically ('Pulled Notion — editorial calendar shows three articles queued for next week: ...').

  Page CREATION is enabled. When the boss asks you to save / write / create / 'drop' / 'spin up' a Notion page summarizing something, use the notion tool's page-creation surface:

  - PARENT — pick by topic, ask if unsure. Use the notion tool to search/list current pages and pick a sensible parent based on what's being saved. Examples of confident routing: kanban / project-status stuff → Project Board subtree; editorial / SEO / article work → Resources subtree (or wherever the boss's editorial calendar lives); Bitmotive client work → Bitmotive subtree; personal notes / journal-ish → the boss's notes area if you can find one. If the topic is unclear or could plausibly live in two places, ASK before writing: 'Wanna drop this under Project Board or Resources, boss?' — short, in voice, one-message. Don't guess on ambiguous saves.
  - TITLE — '<topic> — YYYY-MM-DD'. No 'Bugsy:' prefix, no emoji. Example: 'Open PRs and top-level Notion pages — 2026-05-19'.
...
- **Credential (httpBearerAuth):** `GitHub MCP - PAT`
...
```json
{
  "endpointUrl": "http://mcp-notion:3000/mcp",
  "serverTransport": "httpStreamable",
  "authentication": "bearerAuth",
  "include": "all",
  "options": {}
...
- **Credential (httpBearerAuth):** `Notion MCP - Bearer`
