...
This is the simplest of the Slack surfaces. The more capable [unified workflow](bugsy-unified.md) is what should eventually replace it (memory + RAG), but `bugsy-chat` is the active fallback while the unified flow is being finalized — check n8n for which one is currently active.

## When to use it

- Quick one-shot question, doesn't need context from earlier messages
- Doesn't need facts from the personal knowledge base (no RAG)
- You want the snappiest response time
