...
`agent/n8n/workflows/bugsy-leads-hunter.json` — daily cron, 6:00am CT M-F. Hunts for outbound-sales prospects (companies that look like they'd be good freelance clients) and surfaces the top picks to Slack.

## The pipeline

1. **Schedule Trigger** fires the run.
2. **Twelve SearXNG queries** run sequentially through the self-hosted metasearch container, hitting different signal sources:
   - Greenhouse / Lever / Ashby high-comp engineering postings (companies that pay six figures probably have budget for contract work)
   - TechCrunch + wire-service funding announcements
   - HN "Who's Hiring" threads
   - Wellfound startups
   - BuiltIn Austin (local network)
   - "Tech debt" / "premature ML hire" signals on greenhouse + lever (companies struggling to ship)
   - WeWorkRemotely stack matches
3. **Merge + Deduplicate** combines the result sets and dedupes by URL.
4. **Score Prospects** — `claude-haiku-4-5` rates each candidate 1–10 on Anthony's ICP fit, with a one-line reason. Single batched LiteLLM call.
5. **Parse Scores** maps LLM output back onto each prospect.
6. **Filter ≥4 + Top 5** keeps only candidates scoring ≥4 and trims to the strongest 5.
7. **Prepare Upsert SQL** + **Upsert to Postgres** — store in `agent.leads` with `INSERT ... ON CONFLICT (domain) DO NOTHING`.
8. **Format Slack Message** + **Send to Mulberry Street** — DMs the boss the day's leads.

## Output

Stored in `agent.leads` (see [database reference](../reference/database.md)) and posted to Slack channel "Mulberry Street". Each row carries `score`, matched `signals`, `icp_bucket`, and a `status` (`new` → `contacted` → `replied` → `dead`).

## Tuning

- Add/remove queries by editing the SearXNG HTTP Request nodes. Each is independent — chain order just controls execution sequence.
- Tweak the ICP rubric in the **Score Prospects** Code node's system prompt.
- Score threshold (≥4) and top-N (5) live in **Filter ≥4 + Top 5**.
...
> Auto-generated from `agent/n8n/workflows/bugsy-leads-hunter.json` on 2026-05-18. Run `node agent/n8n/scripts/generate-workflow-reference.mjs` to refresh.
