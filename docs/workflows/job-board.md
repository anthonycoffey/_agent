...
- **External job links open with `noopener noreferrer` and Referrer-Policy: no-referrer** — n8n's default response headers + Cloudflare's defaults can collide with the destination site's COOP, producing `ERR_BLOCKED_BY_RESPONSE` in Chrome. The Respond to Webhook node now sets `Cross-Origin-Opener-Policy: unsafe-none` to keep clicks working.
